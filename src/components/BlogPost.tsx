...
export function BlogPost({ onBack, post, onPostClick }: BlogPostProps) {
  const [activeSection, setActiveSection] = useState("");
  const [showShareMenu, setShowShareMenu] = useState(false);

  const previousPost = post.id > 1 ? getBlogPostById(post.id - 1) : null;
  const nextPost = getBlogPostById(post.id + 1);

  const handlePreviousClick = () => {
    if (previousPost && onPostClick) {
      onPostClick(previousPost.id);
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  };

  const handleNextClick = () => {
    if (nextPost && onPostClick) {
          ...
          <div className="lg:col-span-8">
            ...
            <div className="prose-custom">
              ...
              <div className="my-12 p-6 bg-[#B5E3FF]/5 border-l-4 border-[#B5E3FF] rounded-r-lg">
                ...
                <ul className="space-y-2 text-[#C7CCD9]">
                  <li className="flex items-start gap-3">
                    ...
                    <span>
                      Modern web development requires a solid understanding of core
                  ...
                  <li className="flex items-start gap-3">
                    <span className="text-[#B5E3FF] mt-1">•</span>
                    <span>
                      Always test your code and follow community best practices for
                      maintainability.
                    </span>
                  </li>
                </ul>
              ...
              <div className="flex flex-wrap gap-2 mb-12">
                ...
                {["React", "TypeScript", "Web Development", "Best Practices"].map(
                  (tag) => (
                    <span
                      key={tag}
                ...
                <div className="border-t border-b border-[#C7CCD9]/20 py-8 my-12">
                  <div
                    className={`grid gap-6 ${
                      previousPost && nextPost ? "sm:grid-cols-2" : "sm:grid-cols-1"
                    }`}
                  >
                    {previousPost && (
                      <button
                    ...
                    {nextPost && (
                      <button
                        ...
                        className={`group p-6 bg-[#1A1F35] rounded-lg border border-[#C7CCD9]/20 hover:border-[#B5E3FF]/50 transition-all hover:shadow-lg hover:shadow-[#B5E3FF]/10 ${
                          previousPost ? "text-right" : "text-left"
                        }`}
                      >
                        <div
                          className={`flex items-center gap-2 text-[#B5E3FF] text-sm mb-2 ${
                            previousPost ? "justify-end" : ""
                          }`}
                        >
                          <span>Next Post</span>
                          <ArrowRight
                ...
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[#C7CCD9] text-sm leading-relaxed">
                      {post.author.bio} Passionate about creating educational content
                      and helping developers grow their skills.
                    </p>
                  </div>
                </div>
